...
export default function Sidenav({ role, setRole }) {
  ...
  return (
    // FIX: Added `self-start` here so it doesn't stretch, allowing `sticky` to work.
    <aside className="hidden md:flex flex-col w-64 shrink-0 h-screen bg-white sticky top-0 self-start border-r border-gray-100 shadow-sm z-50">
      ...
      {/* NAVIGATION LINKS (Added overflow-y-auto in case links exceed screen height) */}
      <nav className="flex-grow py-6 flex flex-col gap-1 overflow-y-auto custom-scrollbar">
        {role === 'admin' ? (
          <>
          ...
          <>
            ...
            <Link to="/staff-settings" className={`flex items-center px-6 py-3 transition-all ${isActive('/staff-settings')}`}>Settings</Link>
          </>
        ) : null}
      </nav>
