// src/pages/StaffSettings.jsx
import { useNavigate } from 'react-router-dom';

export default function StaffSettings({ setGlobalRole }) {
  const navigate = useNavigate();

  const handleLogout = () => {
    // Adding a quick confirmation so they don't accidentally click it
    const confirmLogout = window.confirm("Are you sure you want to log out of RentBook?");
    if (confirmLogout) {
      if (setGlobalRole) setGlobalRole(null);
      navigate('/login');
    }
  };

  return (
    <div className="flex flex-col h-full relative bg-gray-50 font-sans min-h-screen">
      <div className="grow overflow-y-auto px-6 pt-12 pb-32 md:pb-12 md:max-w-2xl md:mx-auto md:w-full">
        
        {/* Header */}
        <div className="mb-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
          <h1 className="text-3xl md:text-4xl font-black text-text-main tracking-tight">Settings</h1>
          <p className="text-sm font-medium text-gray-500 mt-1">Manage your account and preferences</p>
        </div>

        {/* Profile Card */}
        <div className="bg-white rounded-[32px] p-6 shadow-sm border border-gray-100 flex items-center gap-5 mb-8 animate-in fade-in slide-in-from-bottom-4 duration-500 delay-75">
          <div className="w-20 h-20 rounded-full bg-linear-to-br from-primary-light to-primary flex items-center justify-center text-white text-2xl font-black shadow-lg shadow-primary/20 shrink-0">
            SA {/* Initials for Staff Associate */}
          </div>
          <div className="grow">
            <h2 className="text-xl font-black text-gray-900">Staff Associate</h2>
            <p className="text-sm text-gray-400 font-bold mb-1">frontdesk@rentbook.app</p>
            <span className="inline-block px-3 py-1 bg-green-50 text-green-600 text-[10px] font-black uppercase tracking-widest rounded-lg">
              Active Shift
            </span>
          </div>
        </div>

        {/* Preferences Section (Mocked UI for realism) */}
        <div className="mb-8 space-y-3 animate-in fade-in slide-in-from-bottom-4 duration-500 delay-100">
          <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest ml-4 mb-4">App Preferences</h3>
          
          <div className="bg-white rounded-3xl p-5 shadow-sm border border-gray-100 flex justify-between items-center cursor-pointer hover:border-gray-200 transition-colors">
            <div className="flex items-center gap-4">
              <div className="p-3 bg-gray-50 rounded-2xl text-gray-500">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" className="w-5 h-5 stroke-[2.5px]"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"></path><path d="M13.73 21a2 2 0 0 1-3.46 0"></path></svg>
              </div>
              <div>
                <p className="font-bold text-gray-900">Push Notifications</p>
                <p className="text-xs text-gray-400 font-medium">Alerts for overdue items</p>
              </div>
            </div>
            {/* Mock Toggle */}
            <div className="w-12 h-6 bg-primary rounded-full relative transition-colors">
              <div className="w-5 h-5 bg-white rounded-full absolute right-0.5 top-0.5 shadow-sm"></div>
            </div>
          </div>

          <div className="bg-white rounded-3xl p-5 shadow-sm border border-gray-100 flex justify-between items-center cursor-pointer hover:border-gray-200 transition-colors">
            <div className="flex items-center gap-4">
              <div className="p-3 bg-gray-50 rounded-2xl text-gray-500">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" className="w-5 h-5 stroke-[2.5px]"><circle cx="12" cy="12" r="5"></circle><line x1="12" y1="1" x2="12" y2="3"></line><line x1="12" y1="21" x2="12" y2="23"></line><line x1="4.22" y1="4.22" x2="5.64" y2="5.64"></line><line x1="18.36" y1="18.36" x2="19.78" y2="19.78"></line><line x1="1" y1="12" x2="3" y2="12"></line><line x1="21" y1="12" x2="23" y2="12"></line><line x1="4.22" y1="19.78" x2="5.64" y2="18.36"></line><line x1="18.36" y1="5.64" x2="19.78" y2="4.22"></line></svg>
              </div>
              <div>
                <p className="font-bold text-gray-900">Theme</p>
                <p className="text-xs text-gray-400 font-medium">System Default (Light)</p>
              </div>
            </div>
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" className="w-5 h-5 text-gray-300 stroke-[3px]"><polyline points="9 18 15 12 9 6"></polyline></svg>
          </div>
        </div>

        {/* Danger Zone */}
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 delay-150">
          <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest ml-4 mb-4">Account Actions</h3>
          <button 
            onClick={handleLogout}
            className="w-full bg-red-50 text-[#bf4a53] flex items-center justify-center gap-3 py-5 rounded-3xl font-black text-lg border border-red-100 hover:bg-[#bf4a53] hover:text-white transition-all active:scale-[0.98]"
          >
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" className="w-5 h-5 stroke-[3px]"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"></path><polyline points="16 17 21 12 16 7"></polyline><line x1="21" y1="12" x2="9" y2="12"></line></svg>
            Log Out Securely
          </button>
        </div>

      </div>
    </div>
  );
}