...
export default function StaffHistory() {
  ...
  // Initialize state AND persist new transactions to the mock array
  const [transactions, setTransactions] = useState(() => {
    if (location.state?.newTransaction) {
      const newTx = location.state.newTransaction;
      // Prevent duplicates in the mock array
  ...
  // Clear the location state so refreshing the page doesn't re-trigger it
  useEffect(() => {
    if (location.state?.newTransaction) {
  ...
  return (
    <div className="flex flex-col h-full relative bg-[#faf6f6]">
                    ...
                    <tr className="hover:bg-gray-50/50 transition-colors group">
                      <td className="px-8 py-5">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            {tx.type === 'wedding' && <span className="text-[10px] font-black text-purple-500 uppercase tracking-widest">Bulk Order</span>}
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-8 py-5 text-right">
                        <div className="flex justify-end gap-2 items-center">
                          {tx.type === 'wedding' && (
                            <button onClick={() => setExpandedTx(expandedTx === tx.txId ? null : tx.txId)} className="p-2 text-gray-400 hover:text-black transition-colors rounded-xl hover:bg-gray-100 flex items-center gap-2 text-xs font-black uppercase tracking-widest mr-2">
                          ...
                          {tx.status !== 'completed' && (
                            <>
                              ...
                              <button onClick={() => openReturnModal(tx)} className="p-2 text-[#34c759] hover:bg-[#34c759] hover:text-white rounded-xl transition-all flex items-center gap-1" title="Mark as Returned">
                      ...
                      <tr className="bg-gray-50/50">
                        <td colSpan="5" className="px-8 py-6">
                          <div className="bg-white rounded-3xl p-6 border border-gray-100 shadow-inner">
                            <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4">Entourage Checklist</p>
                              ...
                              {tx.items?.map((subItem, idx) => (
                                <div key={idx} className="flex justify-between items-center p-3 rounded-xl border border-gray-50 bg-gray-50/50 hover:bg-gray-50 transition-colors">
                                  <div>
                                    ...
                                    <p className="text-[10px] font-bold text-gray-500">{getItemName(subItem.itemId)}</p>
                                  </div>
                                  {subItem.returned ? (
                                    <span className="text-[10px] font-black bg-green-100 text-green-700 px-3 py-1.5 rounded-lg uppercase tracking-widest">Returned</span>
                                  ) : (
                                    <button onClick={() => openSubItemReturnModal(tx, idx, subItem)} className="text-[10px] font-black bg-white border border-gray-200 text-[#111010] hover:border-[#34c759] hover:text-[#34c759] px-3 py-1.5 rounded-lg uppercase tracking-widest transition-colors shadow-sm">
                                      Receive
                                    </button>
                                  )}
                                </div>
              ...
              >
                <div className="p-5 flex items-center gap-4">
                  ...
                  <div className="grow min-w-0">
                    ...
                    <div className="text-[11px] text-[#8e8e93] font-bold truncate mb-1">
                    ...
                    <div className="flex items-center justify-between mt-2">
                      ...
                      <div className="flex gap-1.5">
                        ...
                        {tx.type === 'wedding' && (
                          <button onClick={() => setExpandedTx(expandedTx === tx.txId ? null : tx.txId)} className="w-8 h-8 flex items-center justify-center bg-purple-50 rounded-xl text-purple-500 active:bg-purple-100 transition-colors">
                            <svg className={`w-4 h-4 transition-transform ${expandedTx === tx.txId ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M19 9l-7 7-7-7" /></svg>
                          </button>
                        )}
                ...
                {tx.type === 'wedding' && expandedTx === tx.txId && (
                  <div className="px-5 pb-5 pt-2 border-t border-gray-50 bg-gray-50/30">
                     <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3">Entourage Items</p>
                       ...
                       {tx.items?.map((subItem, idx) => (
                         <div key={idx} className="flex justify-between items-center bg-white p-3 rounded-xl border border-gray-100 shadow-sm">
                            <div className="truncate pr-2">
                              <p className="font-bold text-xs text-[#111010] truncate">{subItem.role}</p>
                              <p className="text-[9px] font-bold text-gray-500 truncate">{getItemName(subItem.itemId)}</p>
                            </div>
                            {subItem.returned ? (
                              <span className="text-[9px] font-black bg-green-100 text-green-700 px-2 py-1 rounded-md uppercase shrink-0">Returned</span>
                            ) : (
                              <button onClick={() => openSubItemReturnModal(tx, idx, subItem)} className="text-[9px] font-black border border-gray-200 text-[#111010] hover:border-[#34c759] hover:text-[#34c759] px-2 py-1 rounded-md uppercase shrink-0">
                                Receive
                              </button>
                            )}
                         </div>
      ...
      {viewingTx && (
        ...
        >
          <div 
            className="bg-white w-full max-w-lg rounded-t-4xl sm:rounded-[40px] shadow-2xl animate-slide-up sm:animate-scale-in max-h-[90vh] overflow-y-auto scrollbar-hide pb-safe"
          ...
          >
            <div className="p-6 sm:p-8">
              ...
              <div className="space-y-4 sm:space-y-6">
                <div className="flex gap-4 p-4 sm:p-5 bg-gray-50 rounded-3xl sm:rounded-3xl border border-gray-100">
                  ...
                  <div className="flex flex-col justify-center">
                    ...
                    <p className="text-[#bf4a53] font-bold text-xs sm:text-sm mt-1">{viewingTx.type === 'wedding' ? 'Bulk Order' : `Item ID: ${viewingTx.itemId}`}</p>
                  </div>
                </div>
                ...
                {viewingTx.status === 'completed' && viewingTx.returnPhotoUrl && (
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="p-4 sm:p-5 bg-green-50/50 rounded-2xl border border-green-100">
                        ...
                        <p className="text-xs sm:text-sm font-medium text-gray-700 italic">
                          "{viewingTx.returnNotes || "Item returned in good condition."}"
                        </p>
                      </div>
                    </div>
                  </div>
              ...
              <div className="space-y-4">
                <div className="relative h-40 sm:h-48 bg-gray-50 rounded-3xl sm:rounded-3xl border-2 border-dashed border-gray-200 flex flex-col items-center justify-center overflow-hidden group">
                  {imagePreview ? (
                    <>
                      ...
                      <button type="button" onClick={() => setImagePreview(null)} className="absolute top-2 right-2 p-2 bg-red-500 text-white rounded-full shadow-lg">
                ...
                <div>
                  ...
                  <textarea 
                    ...
                    className="w-full mt-1 p-3.5 sm:p-4 bg-gray-50 border-none rounded-2xl text-xs sm:text-sm font-medium focus:ring-2 focus:ring-[#34c759] outline-none min-h-20 sm:min-h-25"
                  />
                </div>
              </div>
