...
export default function StaffNewRental() {
  ...
  return (
    <div className="min-h-screen w-full bg-app-bg font-sans flex overflow-hidden">
      
      {/* DESKTOP SIDEBAR */}
      <div className="hidden md:flex md:w-[35%] bg-linear-to-br from-primary-light to-primary p-16 flex-col justify-between text-white">
        <div>
      ...
      {/* MAIN CONTENT AREA */}
      <div className="relative flex flex-col grow h-screen bg-app-bg">
        ...
        <div className="flex px-6 py-4 items-center md:px-12 md:max-w-2xl">
          {[1, 2, 3].map((num) => (
            <div key={num} className="relative flex flex-col items-center flex-1">
              ...
              <span className={`mt-1 text-[10px] md:text-xs font-bold uppercase tracking-wider ${step === num ? 'text-primary' : 'text-gray-400'}`}>
        ...
        {/* Content Area */}
        <div className="grow overflow-y-auto px-6 pt-4 pb-48 md:px-12 scrollbar-hide">
          <div className="max-w-2xl">
            {/* STEP 1: Select Item */}
            {step === 1 && (
              <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                <h2 className="text-[22px] md:text-3xl font-extrabold text-text-main">Select Item</h2>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
                  {CATALOG_ITEMS.filter(i => i.status === 'Available').map(item => (
                    <div 
                      ...
                      className={`bg-white rounded-3xl md:rounded-4xl p-4 flex items-center gap-4 cursor-pointer transition-all border-2 
                        ${selectedItem?.id === item.id ? 'border-primary shadow-md scale-[1.02]' : 'border-transparent shadow-sm hover:border-gray-100'}`}
                    >
                      <img src={item.imageUrl} className="w-15.5 h-15.5 md:w-20 md:h-20 rounded-xl md:rounded-2xl object-cover" alt="" />
                      <div className="grow">
            ...
            {step === 2 && (
              <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                <h2 className="text-[22px] md:text-3xl font-extrabold text-text-main">Customer Info</h2>
                <p className="text-[14px] md:text-base text-gray-500 font-medium mb-6">Enter details or scan government ID</p>
                
                <button className="w-full md:max-w-md bg-linear-to-r from-primary to-primary-light text-white py-4 rounded-[18px] font-bold text-[15px] flex items-center justify-center gap-2 shadow-lg shadow-primary/20 mb-8 active:scale-95 transition-transform">
                ...
                <div className="space-y-5 md:max-w-xl">
                  <div className="flex flex-col gap-2">
                    <label className="text-[13px] font-bold text-text-main ml-1 uppercase tracking-wider">Full Name</label>
                    <input type="text" value={customer.name} onChange={e => setCustomer({...customer, name: e.target.value})} className="w-full p-4 md:p-5 rounded-[18px] bg-white shadow-sm outline-none focus:ring-2 focus:ring-primary/10 text-lg" placeholder="e.g. Liza Soberano" />
                  </div>
                  <div className="flex flex-col gap-2">
                    <label className="text-[13px] font-bold text-text-main ml-1 uppercase tracking-wider">Contact Number</label>
                    <input type="tel" value={customer.contact} onChange={e => setCustomer({...customer, contact: e.target.value})} className="w-full p-4 md:p-5 rounded-[18px] bg-white shadow-sm outline-none focus:ring-2 focus:ring-primary/10 text-lg" placeholder="e.g. 0917-123-4567" />
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
                    <div className="flex flex-col gap-2">
                      <label className="text-[13px] font-bold text-text-main ml-1 uppercase tracking-wider">Rental Date</label>
                    ...
                    <div className="flex flex-col gap-2">
                      <label className="text-[13px] font-bold text-text-main ml-1 uppercase tracking-wider">Return Date</label>
            ...
            {step === 3 && selectedItem && (
              <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                <h2 className="text-[22px] md:text-3xl font-extrabold text-text-main">Confirm Rental</h2>
                <p className="text-[14px] md:text-base text-gray-500 font-medium mb-6">Review details before processing</p>
                
                <div className="bg-white rounded-3xl md:rounded-4xl p-6 md:p-10 shadow-sm border border-gray-50 md:max-w-xl">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex justify-between border-b border-gray-50 pb-3">
                      ...
                      <span className="text-text-main font-bold md:text-lg">{customer.returnDate}</span>
                    </div>
                    <div className="flex justify-between items-center pt-4">
                      <span className="text-[18px] md:text-xl font-black text-gray-400">Total Due</span>
                      <span className="text-3xl md:text-5xl font-black text-primary">₱{selectedItem.baseRate + selectedItem.deposit}</span>
                    </div>
                  </div>
                </div>
              </div>
        ...
        {/* FIXED ACTION BAR */}
        <div className="absolute bottom-0 left-0 right-0 p-6 pb-10 md:p-12 md:bg-transparent bg-white rounded-t-4xl md:rounded-none shadow-[0_-4px_24px_rgba(0,0,0,0.06)] md:shadow-none z-20">
          <div className="max-w-2xl flex flex-col md:flex-row gap-3">
            <button onClick={handleNext} className="flex-2 bg-primary text-white py-5 md:py-6 rounded-[22px] md:rounded-3xl font-bold text-[17px] md:text-xl shadow-xl shadow-primary/20 hover:brightness-110 active:scale-[0.98] transition-all order-1 md:order-2">
              {step === 3 ? '✓ Confirm & Generate Receipt' : 'Continue to Next Step'}
            </button>
            {step > 1 && (
              <button onClick={() => setStep(step - 1)} className="flex-1 py-5 md:py-6 text-gray-400 font-bold text-sm md:text-lg hover:text-text-main transition-colors order-2 md:order-1">
                Go Back
              </button>
            )}
          </div>
